...
var debug = require("debug")("loopback:connector:neo4j-cypher");
...
var Neo4j = function (settings, dataSource) {
    ...
    Connector.call(this, "neo4j-cypher", settings);
 ...
 * @returns {string}
 */
Neo4j.prototype.getSourceModelName = function (model) {
    "use strict";
    var modelClass = this.getModelDefinition(model);
    var sourceModel = model;
    if (modelClass.settings.neo4j) {
        sourceModel = modelClass.settings.neo4j.sourceModel || model;
    }
    return sourceModel;
};
 ...
 * @returns {string}
 */
Neo4j.prototype.getTargetModelName = function (model) {
    "use strict";
    var modelClass = this.getModelDefinition(model);
    var targetModel = model;
    if (modelClass.settings.neo4j) {
        targetModel = modelClass.settings.neo4j.targetModel || model;
    }
    return targetModel;
};
...
/**
 * Create a new graph relation for the given data.
 *
 * @param {string} model - The model name
...
Neo4j.prototype.create = function (model, data, options, callback) {
    "use strict";
    console.log("Inside create function of neo4j-relation");
    var self = this,
        idName = self.getIdName(model),
        sourceModelName = self.getSourceModelName(model),
        targetModelName = self.getTargetModelName(model),
        params = {};
    ...
    self.db.cypher({
        "query": "MATCH (a:"+sourceModelName+" {id: '" + data.sourceId + "'}),(b:"+targetModelName+" {id: '" + data.targetId + "'}) MERGE (a)-[r:"+self.label(model)+" {id: '" + data[idName] + "', sourceId: '" + data.sourceId + "', targetId: '" + data.targetId + "'}]->(b) RETURN r",
    }, function (error, response) {
        console.log("Inside cypher response");
        if (self.debug) {
...
Neo4j.prototype.updateOrCreate = function (model, data, options, callback) {
    ...
    self.db.cypher({
        ...
        response = self.fromDatabase(model, response[0].r.properties);
        if (callback) {
            callback(error, response);
        }
    });
...
Neo4j.prototype.all = function (model, filter, options, callback) {
    ...
    self.db.cypher({
        ...
        if (response) {
            console.log("Inside getAll response of neo4j-cypher");
            if (filter.fields) {
                data = response.map(function (entry) {
